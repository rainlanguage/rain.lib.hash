...
The bytes each function in `LibHashNoAlloc` hashes are its API, not an internal
detail. `hashBytes` hashes the `length` bytes of its argument, `hashWords` the
...
another. Our goal is that contracts implementing these patterns can securely
accept arbitrary inputs of the types they are written for (see "Security of
composition" below), NOT that the basic approach ossifies due to unrelated
contracts doing different things to each other.
